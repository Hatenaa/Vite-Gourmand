...
document.addEventListener('click', function (e) {
  var btn = e.target.closest('[data-spinner-btn]');
  if (!btn) return;
...
window.addEventListener('pageshow', function (event) {
  if (event.persisted) {
    document.querySelectorAll('[data-spinner-btn]').forEach(function (btn) {
...
if (heroImg && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
  window.addEventListener('scroll', function () {
    heroImg.style.transform = "translateY(".concat(window.scrollY * 0.3, "px)");
  }, {
    passive: true
  });
}
...
(function initToggle() {
  var toggle = document.getElementById('menu_isActive');
